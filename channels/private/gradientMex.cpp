...
// compute gradient magnitude and orientation at each location (uses sse)
void gradMagFast( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  int x, y, y1, c, h4, s; float *Gx, *Gy, *M2, *MFast; __m128 *_Gx, *_Gy, *_M2, _m, *_MFast;
  float *acost = acosTable(), acMult=10000.0f;
  // allocate memory for storing one column of output (padded so h4%4==0)
  h4=(h%4==0) ? h : h-(h%4)+4; s=d*h4*sizeof(float);
  M2=(float*) alMalloc(s,16); _M2=(__m128*) M2;
  Gx=(float*) alMalloc(s,16); _Gx=(__m128*) Gx;
  Gy=(float*) alMalloc(s,16); _Gy=(__m128*) Gy;
  MFast=(float*) alMalloc(s,16); _MFast=(__m128*) MFast;
  ...
  for( x=0; x<w; x++ ) {
    ...
    for( y=0; y<h4/4; y++ ) {
      //m=min(abs(gx)+abs(gy),255);
      _MFast[y] = MIN(ADD(ABS(_Gx[y]), ABS(_Gy[y])), SET(255.0f));
    ...
    memcpy( M+x*h, MFast, h*sizeof(float) );    
  }
  alFree(Gx); alFree(Gy); alFree(M2), alFree(MFast);
}
...
void mGradMagFast( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  pl[0] = mxCreateMatrix3(h,w,1,mxSINGLE_CLASS,0,(void**)&M);
  if(nl>=2) pl[1] = mxCreateMatrix3(h,w,1,mxSINGLE_CLASS,0,(void**)&O);
  gradMagFast(I, M, O, h, w, d, full>0 );
}
